...
export default function FineTuneUploader({ agentId, isTraining }: FineTuneUploaderProps) {
  ...
  const handleUpload = async () => {
    ...
    try {
      const response = await fetch(
        `${process.env.NEXT_PUBLIC_API_URL || 'http://localhost:8001'}/agents/${agentId}/finetune/upload`,
        {
          method: 'POST',
          headers: {
  ...
  const handleUpdateModel = async () => {
    ...
    try {
      const response = await fetch(
        `${process.env.NEXT_PUBLIC_API_URL || 'http://localhost:8001'}/agents/${agentId}/model`,
        {
          method: 'PATCH',
          headers: {
